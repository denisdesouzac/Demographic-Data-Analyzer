...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(df['education'].value_counts()['Bachelors'] * 100 / len(df), 1)
    ...
    rich_percentage = round(len(df.loc[(df['salary'] == '>50K') & (df['hours-per-week'] == min_work_hours)]) * 100 / num_min_workers)

    # What country has the highest percentage of people that earn >50K?
    rich_people = df.loc[df['salary'] == '>50K']
    rich_people_per_country_percentage = rich_people['native-country'].value_counts() * 100 / df['native-country'].value_counts()
    
    highest_earning_country = rich_people_per_country_percentage.idxmax()
    highest_earning_country_percentage = round(rich_people_per_country_percentage.max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_in_occupation = rich_people.loc[df['native-country'] == 'India']['occupation'].value_counts().idxmax()
